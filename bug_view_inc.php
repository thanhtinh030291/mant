...
if( !array_diff( $g_transfer_history_ids, $t_related_custom_field_ids ) ) {
    $TransferHistory = json_decode( custom_field_get_value( $g_transfer_history_cid, $f_bug_id ), true );
    if( !empty( $TransferHistory ) ) {
        $t_cfgd_claim_no = custom_field_get_definition( $g_claim_no_cid );
        $t_cfgd_transfer_status = custom_field_get_definition( $g_transfer_status_cid );
        $t_cfgd_transfer_date = custom_field_get_definition( $g_transfer_date_cid );
        $t_cfgd_transfer_amount = custom_field_get_definition( $g_transfer_amount_cid );
        $t_cfgd_transfer_remark = custom_field_get_definition( $g_transfer_remark_cid );
        $t_cfgd_mdtc = custom_field_get_definition( $g_mdtc_cid );
        # Labels
        echo '<tr>';
        echo '<td class="category">', string_display( lang_get_defaulted( $t_cfgd_claim_no[ 'name' ] ) ), '</td>';
        echo '<td class="category">', string_display( lang_get_defaulted( $t_cfgd_transfer_status[ 'name' ] ) ), '</td>';
        echo '<td class="category">', string_display( lang_get_defaulted( $t_cfgd_transfer_date[ 'name' ] ) ), '</td>';
        echo '<td class="category">', string_display( lang_get_defaulted( $t_cfgd_transfer_amount[ 'name' ] ) ), '</td>';
        echo '<td class="category">', string_display( lang_get_defaulted( $t_cfgd_transfer_remark[ 'name' ] ) ), '</td>';
        echo '<td class="category">', string_display( lang_get_defaulted( $t_cfgd_mdtc[ 'name' ] ) ), '</td>';
        echo '</tr>';

        echo '<tr>';
        # Claim No
        echo '<td>', print_custom_field_value( $t_cfgd_claim_no, $g_claim_no_cid, $f_bug_id ), '</td>';
        # Transfer Status
        echo '<td><i class="fa fa-square fa-status-box" style="color:' . $g_transfer_status_color[ string_custom_field_value( $t_cfgd_transfer_status, $g_transfer_status_cid, $f_bug_id ) ] . '"></i> ', print_custom_field_value( $t_cfgd_transfer_status, $g_transfer_status_cid, $f_bug_id ), '</td>';
        # Transfer Date
        echo '<td>', print_custom_field_value( $t_cfgd_transfer_date, $g_transfer_date_cid, $f_bug_id ), '</td>';
        # Transfer Amount
        echo '<td>', number_format(intval(custom_field_get_value($g_transfer_amount_cid, $f_bug_id))),'</td>';
        # Transfer Remark
        echo '<td>', print_custom_field_value( $t_cfgd_transfer_remark, $g_transfer_remark_cid, $f_bug_id ), '</td>';
        # Ma Dien Thanh Cong
        echo '<td>', print_custom_field_value( $t_cfgd_mdtc, $g_mdtc_cid, $f_bug_id ), '</td>';
        echo '</tr>';
        ...
        # Transfer History
        echo '<tr><td class="category">Transfer History</td>';
        echo '<td colspan="6" style="padding: 0px"><table class="table table-bordered table-condensed"><tr>';
        foreach( $g_transfer_history as $hdr ) {
            echo '<td class="bug-description category" style="background-color:#00ffff">' . $hdr . '</td>';
        }
        echo '</tr>';
        foreach( $TransferHistory as $count => $Hist ) {
            echo '<tr ', helper_alternate_class(), '>';
            echo '<td class="bug-description">' . $count . '</td>';
            foreach( $Hist as $val ) {
                echo '<td class="bug-description">' . $val . '</td>';
            }
            echo '</tr>';
        }
        echo '</table></td></tr>';
...
foreach( $t_related_custom_field_ids as $t_id ) {
	...
	if( ( in_array( $t_id, $g_transfer_history_ids ) && $t_id != $g_claim_no_cid ) || in_array( $t_id, array_keys( $g_claim_reasons ) ) || in_array( $t_id, array_values( $g_claim_reasons ) ) || ( $t_id == $g_claim_no_cid && !empty( $TransferHistory ) ) ) {
		continue;
	}
	...
	echo '<tr id="custom_field_' . string_attribute( $t_def['id'] ) . '_row">';
...
$t_custom_field_values = array('claim' => array(), 'date' => array());
foreach( $t_related_custom_field_ids as $t_id ) {
    if( !custom_field_has_read_access( $t_id, $f_bug_id ) ) {
        continue;
    } # has read access

    if(!in_array($t_id, array_keys($g_claim_reasons))) {
        continue;
    }

    $t_custom_fields_found = true;
    $t_def = custom_field_get_definition( $t_id );
    $t_date_def = custom_field_get_definition( $g_claim_reasons[$t_id] );
    $t_custom_field_value = string_custom_field_value( $t_def, $t_id, $f_bug_id );
    $t_custom_field_date_value = string_custom_field_value( $t_date_def, $g_claim_reasons[$t_id], $f_bug_id );

    if( strlen( $t_custom_field_value ) ) {
        $t_custom_field_values['claim'][$t_def['name']] = $t_custom_field_value;
        $t_custom_field_values['date'][$t_def['name']] = $t_custom_field_date_value;
    }
}

if( count( $t_custom_field_values['claim'] ) ) {
    echo '<tr class="spacer"><td colspan="6"></td></tr>';
    echo '<tr class="category"><td class="category" style="text-align: center">Complaint Types</td><td class="category" colspan="3" style="text-align: center">Complaint Reasons</td><td class="category" colspan="2" style="text-align: center">Complaint Date</td></tr>';
}

foreach( $t_custom_field_values['claim'] as $key => $value ) {
    echo '<tr ', helper_alternate_class(), '>';
    echo '<td class="category">', string_display( lang_get_defaulted( $key ) ), '</td>';
    echo '<td colspan="3">', $value, '</td>';
    echo '<td colspan="2">', $t_custom_field_values['date'][$key], '</td>';
    echo '</tr>';
}
