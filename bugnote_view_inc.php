...
<?php }
	...
	for( $i=0; $i < $t_activities_count; $i++ ) {
		...
		$t_file_ids = [];
		if( !empty( $t_activity['attachments'] ) ) {
			foreach( $t_activity['attachments'] as $t_attachment ) {
				$t_file_ids[] = $t_attachment['id'];
			}
		}
...
<tr class="bugnote visible-on-hover-toggle" id="c<?php echo $t_activity['id'] ?>">
		...
		<p class="no-margin small lighter">
			...
			<?php if( $t_activity['private'] ) { ?>
				&#160;&#160;
				<i class="fa fa-eye red"></i> <b class="red"><?php echo lang_get( 'private' ) ?></b>
			<?php } ?>
		</p>
		<p class="no-margin">
			<?php
		...
		<?php
			...
			if( $t_activity['can_change_view_state'] ) {
				if( $t_activity['type'] == ENTRY_TYPE_NOTE  && !$t_bugnote_security_token_state ) {
					$t_bugnote_security_token_state = form_security_token( 'bugnote_set_view_state' );
				} elseif( $t_activity['type'] == ENTRY_TYPE_ATTACHMENT && !$t_bug_file_security_token_state ) {
					$t_bug_file_security_token_state = form_security_token( 'bug_file_set_view_state' );
				}
				...
				if( $t_activity['private'] ) {
					if( $t_activity['type'] == ENTRY_TYPE_NOTE ) {
						print_form_button(
							...
							array( 'private' => '0', 'bugnote_id' => $t_activity['id'], 'file_ids' => implode( ',', $t_file_ids ) ),
							$t_bugnote_security_token_state );
					} elseif( $t_activity['type'] == ENTRY_TYPE_ATTACHMENT ) {
						print_form_button(
							'bug_file_set_view_state.php',
							lang_get( 'make_public' ),
							array( 'private' => '0', 'file_id' => $t_activity['id'], 'file_ids' => implode( ',', $t_file_ids ) ),
							$t_bug_file_security_token_state );
					}
				} else {
					if( $t_activity['type'] == ENTRY_TYPE_NOTE ) {
						print_form_button(
							...
							array( 'private' => '1', 'bugnote_id' => $t_activity['id'], 'file_ids' => implode( ',', $t_file_ids ) ),
							$t_bugnote_security_token_state );
					} elseif( $t_activity['type'] == ENTRY_TYPE_ATTACHMENT ) {
						print_form_button(
							'bug_file_set_view_state.php',
							lang_get( 'make_private' ),
							array( 'private' => '1', 'file_id' => $t_activity['id'], 'file_ids' => implode( ',', $t_file_ids ) ),
							$t_bug_file_security_token_state );
					}
				}
				echo '</div>';
