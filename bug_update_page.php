...
foreach ( $t_related_custom_field_ids as $t_id ) {
	if (in_array($t_id, array_merge(array_keys($g_claim_reasons), array_values($g_claim_reasons)))) {
		continue;
	}
...
$has_complaint_field = false;
foreach ( $t_related_custom_field_ids as $t_id ) {
	if ( !in_array( $t_id, array_keys( $g_claim_reasons ) ) ) {
		continue;
	}
    $has_complaint_field = true;
}

if ( $has_complaint_field ) {
	# spacer
	echo '<tr class="spacer"><td colspan="6"></td></tr>';
	echo '<tr class="category"><td class="category" style="text-align: center">Complaint Types</td><td class="category" colspan="2" style="text-align: center">Complaint Reasons</td><td class="category" colspan="3" style="text-align: center">Complaint Date</td></tr>';
} # custom fields found
...
foreach ( $t_related_custom_field_ids as $t_id ) {
	...
	if ( ( $t_def['display_update'] || $t_def['require_update'] ) && custom_field_has_write_access( $t_id, $t_bug_id ) ) {
		...
		print_custom_field_input( $t_date_def, $tpl_bug_id );
		echo '</td></tr>';
	}
} # foreach( $t_related_custom_field_ids as $t_id )
...
if( current_user_get_access_level() >= DEVELOPER && strpos( $t_project_name, 'UW -' ) !== false ) {
    $t_default_bugnote_view_status = VS_PRIVATE;
}
