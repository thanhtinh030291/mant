...
function bug_activity_get_all( $p_bug_id, $p_include_attachments = true ) {
	...
	foreach( $t_attachments as $t_attachment ) {
		$t_activity = array(
			...
			'private' => $t_attachment['view_state'] != VS_PUBLIC,
			'style' => 'bugnote-note',
		...
		if( $t_bug_readonly ) {
			$t_can_delete_attachment = false;
			$t_can_change_view_state = false;
		} else {
			$t_can_delete_attachment = $t_attachment['can_delete'];

			# check if the user can make this attachment private
			if( $t_user_id == $t_activity['user_id'] ) {
				$t_can_change_view_state = access_has_file_level( $t_bugnote_user_change_view_state_threshold, $t_activity['id'] );
			} else {
		...
		$t_activity['can_delete'] = $t_can_delete_attachment;
	...
	$t_activities = array_merge( $t_files, $t_notes );
