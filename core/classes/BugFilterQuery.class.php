	...
	protected function build_prop_search() {
		...
		foreach( $t_search_terms as $t_search_term => $t_negate ) {
			...
			$t_textsearch_where_clause .= '( ' . $this->sql_like( '{bug}.summary', $c_search )
					...
					. ' OR ' . $this->sql_like( '{bugnote_text}.call_date', $c_search )
					. ' OR ' . $this->sql_like( '{bugnote_text}.call_time', $c_search )
					. ' OR ' . $this->sql_like( '{bugnote_text}.call_type', $c_search )
					. ' OR ' . $this->sql_like( '{bugnote_text}.tel_no', $c_search )
                    . ' OR ' . "( {bug}.id IN ( SELECT DISTINCT bug_id
                                                FROM {custom_field_string}
                                                WHERE value LIKE '%{$c_search}%' ) )";
