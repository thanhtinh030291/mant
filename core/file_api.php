...
function file_attach_files( $p_bug_id, $p_view_state, $p_files ) {
	...
	foreach( $p_files as $t_file ) {
		if( !empty( $t_file['name'] ) ) {
			$t_file_infos[] = file_add( $p_bug_id, $p_view_state, $t_file, 'bug' );
		}
	}
...
function file_get_visible_attachments( $p_bug_id ) {
	...
	for( $i = 0;$i < $t_attachments_count;$i++ ) {
		...
		$t_attachment['view_state'] = $t_view_state;
 ...
 * Set File View State
 * @param integer $p_file_id File identifier.
 * @param integer $p_private File Private identifier.
 * @param string  $p_table   Table identifier.
 * @return boolean
 */
function file_set_view_state( $p_file_id, $p_private, $p_table = 'bug' ) {
	...
	db_param_push();
	$t_query = "UPDATE {bug_file} SET view_state=" . db_param() . " WHERE id=" . db_param();
	db_query( $t_query, array( $t_view_state, $c_file_id ) );
 ...
 * @param integer $p_view_state      File View State.
 ...
 */
function file_add( $p_bug_id, $p_view_state, array $p_file, $p_table = 'bug', $p_title = '', $p_desc = '', $p_user_id = null, $p_date_added = 0, $p_skip_bug_update = false ) {
	$t_file_info = array();
	...
	$c_view_state = (boolean) $p_view_state;
 	# Check for private bugnotes.
 	if( $p_view_state && access_has_bug_level( config_get( 'private_bugnote_threshold' ), $p_bug_id, $c_user_id ) ) {
 		$t_view_state = VS_PRIVATE;
 	} else {
 		$t_view_state = VS_PUBLIC;
 	}
