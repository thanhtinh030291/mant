 ...
 * against that bug
 * @param integer      $p_access_level Integer representing access level.
 * @param integer      $p_issue_attachment_id   Integer representing bug_file id to check access against.
 * @param integer|null $p_user_id      Integer representing user id, defaults to null to use current user.
 * @return boolean whether user has access level specified
 * @access public
 */
function access_has_file_level( $p_access_level, $p_issue_attachment_id, $p_user_id = null ) {
	if( null === $p_user_id ) {
		$p_user_id = auth_get_current_user_id();
	}
	...
	# If the bug is private and the user is not the reporter, then the
	# the user must also have higher access than private_bug_threshold
	if( file_get_field( $p_issue_attachment_id, 'view_state' ) == VS_PRIVATE && file_get_field( $p_issue_attachment_id, 'view_state' ) != $p_user_id ) {
		$t_private_bugnote_threshold = config_get( 'private_bugnote_threshold', null, $p_user_id, $t_project_id );
		$p_access_level = max( $p_access_level, $t_private_bugnote_threshold );
	}
 ...
 * specified bug_file.
 * @param  int $p_access_level   The access level.
 * @param  int $p_bug_file_id     The bug_file id.
 * @param  array $p_user_ids     The array of user ids.
 * @return array filtered array of user ids.
 */
function access_has_file_level_filter( $p_access_level, $p_bug_file_id, $p_user_ids ) {
	$t_users_ids_with_access = array();
	foreach( $p_user_ids as $t_user_id ) {
		if( access_has_file_level( $p_access_level, $p_bug_file_id, $t_user_id ) ) {
			$t_users_ids_with_access[] = $t_user_id;
		}
	}
...
/**
 * Check if the user has the specified access level for the given file
 * and deny access to the page if not
 * @see access_has_bug_file_level
 * @param integer      $p_access_level Integer representing access level.
 * @param integer      $p_file_id      Integer representing file id to check access against.
 * @param integer|null $p_user_id      Integer representing user id, defaults to null to use current user.
 * @access public
 * @return void
 */
function access_ensure_file_level( $p_access_level, $p_file_id, $p_user_id = null ) {
	if( !access_has_file_level( $p_access_level, $p_file_id, $p_user_id ) ) {
		access_denied();
	}
}
