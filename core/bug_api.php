...
function bug_get_attachments( $p_bug_id ) {
	db_param_push();
 	...
 	if ( !access_has_bug_level( config_get( 'private_bugnote_threshold' ), $p_bug_id ) ) {
 		$t_restriction = ' AND ( view_state=' . VS_PUBLIC . ')';
 	} else {
 		$t_restriction = '';
 	}

	$t_query = 'SELECT id, title, view_state, diskfile, filename, filesize, file_type, date_added, user_id
		                FROM {bug_file}
		                WHERE bug_id=' . db_param() . $t_restriction . '
		                ORDER BY date_added';
