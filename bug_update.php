...
foreach ( $t_related_custom_field_ids as $t_cf_id ) {
	...
	if( !gpc_isset_custom_field( $t_cf_id, $t_cf_def['type'] ) ) {
		...
		if( in_array( $t_cf_id, array_keys( $g_claim_reasons ) ) ) {
			$t_date_def = custom_field_get_definition( $g_claim_reasons[$t_cf_id] );
			if( !gpc_isset_custom_field( $g_claim_reasons[$t_cf_id], $t_date_def['type'] ) ) {
				error_parameters( lang_get_defaulted( custom_field_get_field( $g_claim_reasons[$t_cf_id], 'name' ) ) );
				trigger_error( ERROR_EMPTY_FIELD, ERROR );
			}
		}
	}
